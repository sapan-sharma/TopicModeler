...
$(".drag").show();
	...
	multipleDND = chkInteractionType();
	clone = ChkMultiDrags();
...
var dgId;
	...
	var rtnValue = compareArray(answerArray[dpId],dgId);
	return rtnValue;
	}
}
...
var splitVal = $(this).attr('id').split("_")[2];
userAnswerArray[index].push(parseFloat(splitVal));
}else{
userAnswerArray[index].push(parseFloat($(this).attr('id')));
}
});
}
...
function checkIfSameDragObjectIsDropped(dgObj,dpObj){
    if (trackObjects[SeqID].singleqtnperpage == "no") {
        ...
        if (dpObjChildren > 0) {
            //alert($(dgObj).attr("id"));
            var dgObjId = $(dgObj).attr("id").split("_")[2];
            $(dpObj).children().each(function () {
                var dropObjIds = $(this).attr("id").split("_")[2];
                if (dropObjIds == dgObjId) {
                    dragMatched = false;
                }
            })
...
function appendToDrop($currentDragObj, $currentDropObj, val) {
    if (trackObjects[SeqID].singleqtnperpage == "no") {
...
function chkInteractionType(){
var arrLen = answerArray.length;
var tempVar = false;
...
function dragDropInit() {
    $('.drag').data('isDropped', false);
    $('.drop').data('isDropped', false);
    if (trackObjects[SeqID].singleqtnperpage == "no") {
        // singlepageinit();
        userAnswerArray = new Array();
        mobileDragsArray = [];
...
var text = "Please select the drag item(s) that you want to drop in to the box"
 ...
 $tbl.append("<tr><td colspan='2'><input type='submit' onClick='appendSelectedDrops()' value='OK'></td></tr>");
...
function checkfordisableOnDropped() {
    if (trackObjects[SeqID].singleqtnperpage == "no") {
